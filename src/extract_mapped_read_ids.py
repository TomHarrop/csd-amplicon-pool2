...
hvr_coverage = float(snakemake.params['hvr_coverage'])
...
logging.info(
  f'Extracting reads with > {int(hvr_coverage * 100)}% coverage of HVR exon')
kept_ids = sorted(set(
  x.qname for x in span_iter
  if (x.get_overlap(hvr_start, hvr_stop) / hvr_size > hvr_coverage
      and len(x.seq) < 500
      and len(x.seq) > 370
      and x.mapq > 30)))
